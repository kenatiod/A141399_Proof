...
Let $r\ge 37$.  Suppose that $m(m+1)$ is prime-complete of order $r$.  Let $D$ be the 
...
Since $r \ge 37 \ge 10$, Proposition~\ref{prop:index-cap} applies and gives
\[
        n \le p_r+1,
\]
...
Let $r \ge 37$ and let $D \mid P_r$ be squarefree.  If
\[
        R_r(D)>p_r+1,
\]
...
a prime-complete product of order $r \ge 37$ can exist only if there is a 
...
The proof of the index cap only needs $r \ge 10$, but the theorem is stated for $r \ge 37$ 
because that is the intended tail range after the finite computational base.  
Nothing in the proof depends on the specific value $37$ except the inequality $r \ge 10$.
\end{remark}
